...
class icon_panel:
	...
	# How Icon panel should work
	#
	# Give it number of widget rows/columns to display, plus scroll up/down graphics
	# create a list of widgets would should be displayed, in order they should be displayed
	# show the maximum number of icons you have spaces for.
	# show blank icons for any empty spaces
	# show scroll bars if more icons are available than are capable of being displayed
	# keep track of which rows are currently being displayed
	# change only the images which have to be changed during room diplay routine
	# don't call pygame.flip() until all of the changed widgets are set
	
	# How the inventory should look to the user
	#
	# display all items, in order
	# leave blank spaces if not all spaces are filled
	# when more items are available than can be displayed, an arrow pointing down is displayed
	# when the user clicks on the down arrow, the top row is no longer displayed
	# the row previously on the second line is now on the first
	# the bottom row (the third row of the grid) is displayed, along with an up arrow
	# user click on up arrow: rows are shifted down
	# something is removed from inventory: all items are shuffled up and left
	#   if the bottom row is now blank, the inventory is automatically shifted up and no
	#   arrow point is displayed
	
	# How this should be managed in memory
	#
	# max x and max y widgets are created and displayed as blank
	# a list keeps track of all of the icons which should be displayed
	# a variable keeps track of which "row of icons" should be displayed at the top of the panel
	# panel runs a check to make sure it shouldn't auto-scroll up
	# arrows are drawn according to the current display state
