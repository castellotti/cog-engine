...
class CogEngine:
	...
	def display_introduction(self):
		...
		self.output_text(self.gameInformation.introduction_text, self.text_to_speech_enabled)


	#####################################################################

	def display_room(self, room, speak_text=0):
	...
	def parse_command_line(self, command):
		...
		if (self.gameInformation.debug_mode):
			if ( ((verb == "warp") or (verb == "xyzzy"))
				and not(remainder == "") ):
				self.room_warp(remainder)
				command_executed = 1

			if (verb == "execute"):
				self.execute_effect(remainder)
				self.display_room(self.playerInformation.current_room)
				command_executed = 1
		...
		if (not command_executed):
			if ( (remainder == "") and ( (verb == "look") or (verb == "l") ) ):
				...
				self.display_room(self.playerInformation.current_room, speak_text=1)
	...
	def display_verbs(self):
		...
		if ((not show_get) and (show_drop)):
			self.output_test(", Drop, and Last.")
		...
		if ( (direction in self.roomData[ self.playerInformation.current_room ].direction.keys())
			...
			# Path is not obstructed
				...
				if ( (type(self.roomData[self.playerInformation.current_room].direction[direction].first_transition_text) != type(None))
					...
					self.output_text("\n " + self.roomData[self.playerInformation.current_room].direction[direction].first_transition_text, self.text_to_speech_enabled)
				...
				if ( (type(self.roomData[self.playerInformation.current_room].direction[direction].transition_text) != type(None))
					...
					self.output_text("\n " + self.roomData[self.playerInformation.current_room].direction[direction].transition_text, self.text_to_speech_enabled)
	...
	def execute_effect(self, effect):
		...
		if (word == "Adds"):
			...
			# We're adding an Item
				...
				if (word == "CurrentRoom"):
					self.add_item_to_room(self.playerInformation.current_room, item_number)
				if (word[0:4] == "Room"):
					room_number = string.atoi(word[5:-1])
					self.add_item_to_room(room_number, item_number)
				...
				if (word[0:4] == "Room"):
					...
					self.add_obstruction_to_room(room_number, direction_number, obstruction_number)
		...
		if (word == "Removes"):
			...
			# We're removing an Item
				...
				if (word[0:4] == "Room"):
					room_number = string.atoi(word[5:-1])
					self.remove_item_from_room(room_number, item_number)
				...
				if (word[0:4] == "Room"):
					...
					self.remove_obstruction_from_room_direction(room_number, direction_number, obstruction_number)
		...
		if (word == "Modifies"):
			...
			if (word == "Player"):
				...
				if (word[0:12] == "PlayerPoints"):
					self.playerInformation.points = self.evaluate_expression(expression, self.playerInformation.points, number)
				if (word[0:9] == "PlayerExp"):
					self.playerInformation.experience = selfevaluate_expression(expression, self.playerInformation.experience, number)
				if (word[0:8] == "PlayerHP"):
					self.playerInformation.hp = self.evaluate_expression(expression, self.playerInformation.hp, number)
				if (word[0:8] == "PlayerMP"):
					self.playerInformation.mp = self.evaluate_expression(expression, self.playerInformation.mp, number)
				if (word[0:9] == "PlayerStr"):
					self.playerInformation.strength = self.evaluate_expression(expression, self.playerInformation.strength, number)
				if (word[0:8] == "PlayerIQ"):
					self.playerInformation.intelligence = self.evaluate_expression(expression, self.playerInformation.intelligence, number)
				if (word[0:9] == "PlayerDex"):
					self.playerInformation.dexterity = self.evaluate_expression(expression, self.playerInformation.dexterity, number)
				if (word[0:10] == "PlayerAgil"):
					self.playerInformation.agility = self.evalutate_expression(expression, self.playerInformation.agility, number)
				if (word[0:14] == "PlayerCharisma"):
					self.playerInformation.charisma = self.evaluate_expression(expression, self.playerInformation.charisma, number)
				if (word[0:16] == "PlayerArmorLevel"):
					self.playerInformation.armor_level = self.evaluate_expression(expression, self.playerInformation.armor_level, number)
				if (word[0:19] == "PlayerCurrentWeight"):
					self.playerInformation.current_weight = self.evaluate_expression(expression, self.playerInformation.current_weight, number)
		...
		if (word[0:11] == "TextMessage"):
			...
			while (not word[-1] == "]"):
				current_textmessage = current_textmessage + word + " "
				word = effect_word_list[0]
				del(effect_word_list[0])

			current_textmessage = current_textmessage + word[:-1]
			self.output_text(current_textmessage, self.text_to_speech_enabled)
	...
	def get_item(self, item_name):
		...
		if ( (item_name == "all") or (item_name == "everything") ):
			...
			if (type(self.roomData[self.playerInformation.current_room].items) == type(None)):
				...
				for each in string.split(self.roomData[self.playerInformation.current_room].items, ', '):
					...
					self.get_item(self.itemData[ string.atoi(each) ].name)

		if (item_number != -2):
			item_number = self.resolve_item_name(item_name, "CurrentRoom")
		if ((item_number == 0) and not (get_all)):
			item_number = self.find_item_alias(item_name, "CurrentRoom")
		...
		if (item_number == 0):
			...
			effect_string = self.resolve_event("Get", object_list)
	...
	def drop_item(self, item_name):
		...
		if ( (item_name == "all") or (item_name == "everything") ):
			...
			for each in item_list:
				...
				self.drop_item(self.itemData[each].name)

		if (item_number != -2):
			item_number = self.resolve_item_name(item_name, "Inventory")
		if ((item_number == 0) and not (drop_all)):
			item_number = self.find_item_alias(item_name, "Inventory")
		...
		if (item_number == 0):
			...
			effect_string = self.resolve_event( "Drop", [["Item", item_number]] )
				...
				for alias in string.split(self.itemData[item].aliases, ', '):
					...
					if (string.lower(alias) == item_name):
						...
						if (location == "CurrentRoom"):
							if ( not(item_found) and self.room_contains_item(self.playerInformation.current_room, item) ):
								if (self.gameInformation.debug_mode):
									print "Item \"%s\" found." % self.itemData[item].name
								item_found = 1
								item_number = item
							else:
								if (self.room_contains_item(self.playerInformation.current_room, item) ):
				...
				for alias in string.split(self.obstructionData[obstruction].aliases, ', '):
					...
					if (string.lower(obstruction_name) == string.lower(alias)):
						...
						if (location == "CurrentRoom"):
							...
							if ((not obstruction_found) and self.room_contains_obstruction(self.playerInformation.current_room, obstruction) ):
								...
								if ( self.room_contains_obstruction(self.playerInformation.current_room, obstruction) ):
	...
	def parse_command_line_objects(self, remainder):
		...
		object1 = self.parse_object(remainder, "Beginning")
		...
		object2 = self.parse_object(remainder, "End-Exclusive") # see note in parse_objects comments for more information about "End-Exclusive"
	...
	def parse_object(self, phrase, search_focus):
		...
		while ((scratch != "") and not(object_found)):
			...
			# Check Obstructions
			object_number = self.resolve_obstruction_name(scratch, "CurrentRoom")
			if (object_number == 0):
				object_number = self.find_obstruction_alias(scratch, "CurrentRoom")
			if (object_number > 0):
			...
			# Check Items
			if (object_number == 0):
				object_number = self.resolve_item_name(scratch, "CurrentRoom")
			if (object_number == 0):
				object_number = self.resolve_item_name(scratch, "Inventory")
			if (object_number == 0):
				object_number = self.find_item_alias(scratch, "CurrentRoom")
			if (object_number == 0):
				object_number = self.find_item_alias(scratch, "Inventory")
			if (object_number > 0):
				if (not object_found):
				...
				for event in event_keys:
					...
					if ( not(event_found) and (type(self.verbData[verb].events[event]) != type(None)) ):
						...
						or ( ((type(self.verbData[verb].events[event].object2) == type(None)) or
							...
							if ( self.requirements_met(self.verbData[verb].events[event].requirements) ):
	...
	def requirements_met(self, requirements):
		...
		if ((type(requirements) == type(None)) or (requirements == "")):
			...
			if (word[0:6] == "Player"):
				...
				if (word[6:12] == "Points"):
					all_good = self.make_comparison(comparison, self.playerInformation.points, value)
				if (word[6:9] == "Exp"):
					all_good = self.make_comparison(comparison, self.playerInformation.experience, value)
				if (word[6:8] == "HP"):
					all_good = self.make_comparison(comparison, self.playerInformation.hp, value)
				if (word[6:9] == "MP"):
					all_good = self.make_comparison(comparison, self.playerInformation.mp, value)
				if (word[6:9] == "Str"):
					all_good = self.make_comparison(comparison, self.playerInformation.strength, value)
				if (word[6:8] == "IQ"):
					all_good = self.make_comparison(comparison, self.playerInformation.intelligence, value)
				if (word[6:9] == "Dex"):
					all_good = self.make_comparison(comparison, self.playerInformation.dexterity, value)
				if (word[6:10] == "Agil"):
					all_good = self.make_comparison(comparison, self.playerInformation.agility, value)
				if (word[6:14] == "Charisma"):
					all_good = self.make_comparison(comparison, self.playerInformation.charisma, value)
				if (word[6:16] == "ArmorLevel"):
					all_good = self.make_comparison(comparison, self.playerInformation.armor_level, value)
				if (word[6:17] == "CurrentWeight"):
					all_good = self.make_comparison(comparison, self.playerInformation.current_weight, value)
			...
			if (word[0:4] == "Item"):
				...
				if (string.find(word, "ExistsInRoom") != -1):
					...
					all_good = self.room_contains_item(room_number, item_number)
				if (string.find(word, "Weight") != -1):
					comparison = string.split(string.split(word, '[')[1], ']')[0]
					value = string.atoi(string.split(string.split(word, '(')[2], ')')[0])
					all_good = self.make_comparison(comparison, self.itemData[item_number].weight, value)
				if (string.find(word, "Bulk") != -1):
					comparison = string.split(string.split(word, '[')[1], ']')[0]
					value = string.atoi(string.split(string.split(word, '(')[2], ')')[0])
					all_good = self.make_comparison(comparison, self.itemData[item_number].bulk, value)
			...
			if (word[0:11] == "Obstruction"):
				...
				if (string.find(word, 'ExistsInRoom') != -1):
					...
					if (string.find(word, "Direction") != -1):
						...
						all_good = self.room_contains_obstruction(room_number, obstruction_number)
			...
			if (requirements_list != []):
				...
				if (string.lower(word) == "and"):
					...
					all_good = (all_good) and self.requirements_met(string.join(requirements_list, ' '))
				elif (string.lower(word) == "or"):
					del(requirements_list[0]) # remove "or" from requirements string
					all_good = (all_good) or self.requirements_met(string.join(requirements_list, ' '))
		...
		if (type(resolved_object) != type(None)):
			...
			if (resolved_object[0] == "Item"):
				...
				(self.itemData[item_number].description != "")):
					self.output_text("\n\n" + self.itemData[item_number].description, self.text_to_speech_enabled)
			...
			if (resolved_object[0] == "Obstruction"):
				...
				(self.obstructionData[obstruction_number].visible) ):
					self.output_text("\n\n" + self.obstructionData[obstruction_number].description, self.text_to_speech_enabled)
